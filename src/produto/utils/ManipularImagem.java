...
public class ManipularImagem {
    ...
    public static byte[] getImgBytes(BufferedImage image) throws Exception {
        ByteArrayOutputStream bytesImagem = new ByteArrayOutputStream();
        try {
            ImageIO.write(image, "JPEG", bytesImagem);
        } catch (IOException ex) {
            throw new Exception("Falha ao converter os bytes da imagem!");
        }

        //InputStream stream = new ByteArrayInputStream(bytesImagem.toByteArray());

        return bytesImagem.toByteArray();
    }

    //Novo método para exibir imagem na tela
    //Recebe o label que queremos exibir E a imagem como array de bytes do banco
    public static void exibiImagemLabel(byte[] imagemBytes, javax.swing.JLabel label) {
        //primeiro verifica se tem a imagem
        //se tem convert para inputstream que é o formato reconhecido pelo ImageIO

        if (imagemBytes != null) {
            InputStream input = new ByteArrayInputStream(imagemBytes);
            try {
